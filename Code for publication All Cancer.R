...
BloodNumber <- sum(Diabetic_All_Tab$BloodDiabetic)#Number of prediagnosis Diagnostic HBA1c
HesNumber<- sum(Diabetic_All_Tab$HESDiabetic)# Number of coded diabetics
AllNumber<- sum(Diabetic_All_Tab$AnyDiabetes) # Number of diabetics including HBA1c and Coding
BothNumber<- nrow(dplyr::filter(Diabetic_All_Tab, HESDiabetic ==1 & BloodDiabetic ==1)) # Number with both blood and coding
AllCancer<- nrow(Diabetic_All_Tab) # Number of PAtients in total
...
# Post Diagnosis
#Identification of Diabetic Patients Using Each method in All Cancer

Diabetic_All_Tab<- Diabetic_All_Tab %>% dplyr::mutate( Late.HESDiabetic = ifelse( Late.T1Diabetes == 1|
                                                                                    Late.T2Diabetes ==1|
                                                                                    Late.OtherDiabetes == 1, 1, 0),
                                                       Late.BloodDiabetic = Late.Diagnostic) # add label for late effect clinical coding and bloods

Late.BloodNumber <- sum(Diabetic_All_Tab$Late.BloodDiabetic, na.rm = T) #number of late blood diabetics
Late.HesNumber<- sum(Diabetic_All_Tab$Late.HESDiabetic, na.rm = T) # number of late coded diabetics
Late.AllNumber<- sum(Diabetic_All_Tab$Late.AnyDiabetes) # total number of late diabetics
Late.BothNumber<- nrow(dplyr::filter(Diabetic_All_Tab, Late.HESDiabetic ==1 & Late.BloodDiabetic ==1)) # number of diabetics with both
AllCancer<- nrow(Diabetic_All_Tab) # total number of patients
...
Diabetic_All_Tab<- Diabetic_All_Tab %>% dplyr::mutate( TotalHESDiabetic = ifelse( T1Diabetes == 1|
                                                                                    T2Diabetes ==1|
                                                                                    OtherDiabetes == 1 |
                                                                                    Late.T1Diabetes ==1| 
                                                                                    Late.T2Diabetes ==1|
...
TotalBloodNumber <- sum(Diabetic_All_Tab$TotalBloodDiabetic, na.rm = T)
TotalHesNumber<- sum(Diabetic_All_Tab$TotalHESDiabetic)
TotalAllNumber<- sum(Diabetic_All_Tab$TotalAnyDiabetes)
TotalBothNumber<- nrow(dplyr::filter(Diabetic_All_Tab, TotalHESDiabetic ==1 & TotalBloodDiabetic ==1))
...
#Merge Summary Tables to create all numbers before and after in one table
...
Numbers_Bar_plot<- ggplot(OverallDiabSumTab) + 
  ...
  ggtitle("Comparison of Diabetic Breast Cancer Patient Numbers by Method of Identification and Time Period") +
...
Perc_Bar_plot<- ggplot(OverallDiabSumTab) + 
  ...
  theme(text = element_text(size=30),
        axis.text.x = element_text(angle = 90), legend.position = "none") +
  ggtitle("Comparison of Diabetic Breast Cancer Patient Percentage by Method of Identification and Time Period") +
...
AllBloodOnly<- nrow(dplyr::filter(Diabetic_All_Tab, (Diagnostic == 1 | Late.Diagnostic == 1) &
                                    T1Diabetes == 0 & T2Diabetes ==0 & OtherDiabetes ==0 &
                                    Late.T1Diabetes ==0 & Late.T2Diabetes ==0 & Late.OtherDiabetes ==0))

AllCodedOnly<- nrow(dplyr::filter(Diabetic_All_Tab,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 |
                                                         Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                    Diagnostic == 0 & Late.Diagnostic == 0 ))

AllBoth <- nrow(dplyr::filter(Diabetic_All_Tab,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 |
                                                     Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                (Diagnostic == 1 | Late.Diagnostic == 1 )))


PreBloodOnly<- nrow(dplyr::filter(Diabetic_All_Tab, (Diagnostic == 1 ) &
                                    T1Diabetes == 0 & T2Diabetes ==0 & OtherDiabetes ==0))

PreCodedOnly<- nrow(dplyr::filter(Diabetic_All_Tab,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 ) &
                                    Diagnostic == 0 ))

PreBoth <- nrow(dplyr::filter(Diabetic_All_Tab,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 ) &
                                (Diagnostic == 1 )))


PostBloodOnly<- nrow(dplyr::filter(Diabetic_All_Tab, (Late.Diagnostic == 1) &
                                     Late.T1Diabetes ==0 & Late.T2Diabetes ==0 & Late.OtherDiabetes ==0))

PostCodedOnly<- nrow(dplyr::filter(Diabetic_All_Tab,( Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                     Late.Diagnostic == 0 ))

PostBoth <- nrow(dplyr::filter(Diabetic_All_Tab, (Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                 Late.Diagnostic == 1 ))
...
Diabetic_All_Tab<- Diabetic_All_Tab %>%
  dplyr::mutate(Sex_Label = ifelse(Sex == 781, "Female", "Male")) 

HBA1C_Pre<- Diabetic_All_Tab%>% dplyr::filter(Diagnostic == 1)
Coded_Pre<- Diabetic_All_Tab%>% dplyr::filter(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes == 1,
                                              is.na(Sex_Label)==F)
Any_Pre<- Diabetic_All_Tab%>% dplyr::filter(AnyDiabetes == 1,
                                            is.na(Sex_Label)==F)

Demographics_Comp_tab<- data.frame(cbind(c("Median Age", "Standard Deviation Age", "Median IMD",  "Female", "Male" ),
                                         c(median(HBA1C_Pre$Age), sd(HBA1C_Pre$Age), median(HBA1C_Pre$IMDQuintile, na.rm = T), as.vector(summary(as.factor(HBA1C_Pre$Sex)))),
                                         c(median(Coded_Pre$Age), sd(Coded_Pre$Age),median(Coded_Pre$IMDQuintile, na.rm = T), as.vector(summary(as.factor(Coded_Pre$Sex)))),
                                         c(median(Any_Pre$Age), sd(Any_Pre$Age), median(Any_Pre$IMDQuintile, na.rm = T), as.vector(summary(as.factor(Any_Pre$Sex))))))



Unique_HBA1c<- Diabetic_All_Tab %>%
...
Unique_Coding<- Diabetic_All_Tab %>%
...
Both_Identified<- Diabetic_All_Tab %>%
                    dplyr::filter(Diagnostic==1 &
                                  (T1Diabetes == 1 |
                                     T2Diabetes == 1 |
                                     OtherDiabetes == 1),is.na(Sex_Label)==F)%>%
...
Demographics_Unique_tab<- data.frame(cbind(c("Median Age", "Standard Deviation Age", "Median IMD",  "Female", "Male" ),
                                         c(median(Unique_HBA1c$Age), sd(Unique_HBA1c$Age), median(Unique_HBA1c$IMDQuintile, na.rm = T), as.vector(summary(as.factor(Unique_HBA1c$Sex)))),
                                         c(median(Unique_Coding$Age), sd(Unique_Coding$Age),median(Unique_Coding$IMDQuintile, na.rm = T), as.vector(summary(as.factor(Unique_Coding$Sex)))),
                                         c(median(Both_Identified$Age), sd(Both_Identified$Age), median(Both_Identified$IMDQuintile, na.rm = T), as.vector(summary(as.factor(Both_Identified$Sex))))))
...
HB_Sex_Label<- wilcox.test(Sex_Label_Numeric ~ Label, data=HB)
...
CB_Sex_Label<- wilcox.test(Sex_Label_Numeric ~ Label, data=CB)
#HBA1c vs Coding
...
HC_Sex_Label<- wilcox.test(Sex_Label_Numeric ~ Label, data=HC)
...
Stats_Tests<- cbind(c("HBA1c Only VS Coding Only", "HBA1c Only VS HBA1c and Coding", "Coding Only VS HBA1c and Coding"),
              ...
              c(HC_Sex_Label$p.value, HB_Sex_Label$p.value, CB_Sex_Label$p.value))

colnames(Stats_Tests)<- c("Comparitor_Groups", "Age", "IMD Quintile", "Sex_Label")
...
All_Surv <- Surv(time = Diabetic_All_Tab$SurvivalTime/365, event = Diabetic_All_Tab$SurvivalStatus)

HBA1c_KMFit<- survfit(All_Surv ~ Diagnostic  , data = Diabetic_All_Tab)
HBA1c_KM<- ggsurvplot(HBA1c_KMFit, data = Diabetic_All_Tab, risk.table = T,
                      legend.title = "",
                      ...
                      title = "Breast Cancer Survival by HBA1c Only Diabetic Status" ) 

Coded_KMFit<- survfit(All_Surv ~ HESDiabetic  , data = Diabetic_All_Tab)
Coded_KM<- ggsurvplot(Coded_KMFit, data = Diabetic_All_Tab, risk.table = T,
                      legend.title = "",
                      ...
                      ggtheme = theme_bw(),
                      break.time.by=1, xlab = "Time (Years)",
                      title = "Breast Cancer Survival by Clinical Coding Only Diabetic Status" ) 

Any_KMFit<- survfit(All_Surv ~ AnyDiabetes  , data = Diabetic_All_Tab)
Any_KM<- ggsurvplot(Any_KMFit, data = Diabetic_All_Tab, risk.table = T,
                    legend.title = "",
                    ...
                    ggtheme = theme_bw(),
                    break.time.by=1, xlab = "Time (Years)",
                    title = "Breast Cancer Survival by Hybrid Definition Diabetic Status" ) 

surv_median(Coded_KMFit)
surv_median(HBA1c_KMFit)
surv_median(Any_KMFit)

HESGroup<- Diabetic_All_Tab %>% dplyr::filter (HESDiabetic ==1) %>%
  dplyr::mutate(SurvivalComp = "HES Only")
BloodsGroup<- Diabetic_All_Tab %>% dplyr::filter (Diagnostic ==1) %>%
  dplyr::mutate(SurvivalComp = "Bloods Only")
BothGroup<- Diabetic_All_Tab %>% dplyr::filter (Diagnostic ==1 | HESDiabetic ==1) %>%
  dplyr::mutate(SurvivalComp = "Both")
AllCancers<- Diabetic_All_Tab%>% dplyr::mutate(SurvivalComp = "All Cancer Patients")
...
Comp_KM<- ggsurvplot(Comp_KMFit, data = Surv_Comp_Tab, risk.table = T,
                     ...
                     legend.labs = c("Total Cancer Population", "HBA1c Only Diabetics", "Hybrid Definition Diabetics", "Clinical Coding Only Diabetics"),
                     surv.median.line = "hv"  ,
                     font.x = c(20),
                     font.y = c(20),
                     ...
                     #font.main = c(10),
                     font.legend = c(10),
                     base_size = c(10), tables.theme = theme_cleantable(),risk.table.ylab = c(10)) 

Comp_KM$table<- Comp_KM$table + theme(axis.text.y = element_text(size = 10)) 
...
Cox_HES<- survfit(coxph(Surv(time = SurvivalTime/365, event = SurvivalStatus)~Age5 + Sex + IMDQuintile, data = HESGroup))
Cox_Bloods<- survfit(coxph(Surv(time = SurvivalTime/365, event = SurvivalStatus)~Age5 + Sex + IMDQuintile, data = BloodsGroup))
Cox_Both<- survfit(coxph(Surv(time = SurvivalTime/365, event = SurvivalStatus)~Age5 + Sex + IMDQuintile, data = BothGroup))
Cox_All<- survfit(coxph(Surv(time = SurvivalTime/365, event = SurvivalStatus)~Age5 + Sex + IMDQuintile, data = AllCancers))
...
Cox_Comp<- ggsurvplot_combine(fit = list(Cox_HES, Cox_Bloods, Cox_Both, Cox_All), 
                   data = Diabetic_All_Tab,
                   risk.table = T,
                   legend.title = "",
                   legend.labs = c("Clinical Coding Only Diabetics", "HBA1c Only Diabetics", "Hybrid Definition Diabetics ", "Total Cancer Population" ),
                   conf.int = T,
                   censor.shape= NA,
                   xlim= c(0,10),
                   ggtheme = theme_bw(),
                   ...
                   #font.legend = c(20),
                   #base_size = c(30), 
                   tables.theme = theme_cleantable(),risk.table.ylab = c(20)) 

Cox_Comp_Print<- print(Cox_Comp)

#Change in diagnosis date

BothDiab<- Diabetic_All_Tab%>% dplyr::filter((Diagnostic == 1 | Late.Diagnostic ==1) & 
  ...
  dplyr:: mutate(ComorbShiftReverse = ifelse(HESDiabetic == 1 & Diagnostic ==0, 1, 0))

Perc_Shift<- sum(BothDiab$ComorbShift)/nrow(BothDiab)
Perc_Reverse<- sum(BothDiab$ComorbShiftReverse)/nrow(BothDiab)
Perc_ShiftIMD<- BothDiab%>% group_by(IMDQuintile) %>% dplyr::summarise( sum(ComorbShift)/dplyr::n())
Perc_ShiftAge<- BothDiab%>% group_by(Age5) %>% dplyr::summarise( sum(ComorbShift)/dplyr::n())
Pec_ShiftSex_Label<- BothDiab%>% group_by(Sex_Label) %>% dplyr::summarise( sum(ComorbShift)/dplyr::n())
Perc_6Months<- nrow(filter(BothDiab, DiabeticDateDiff<365.25/2 & DiabeticDateDiff>-365.25/2))/nrow(BothDiab)
Perc_1year<- nrow(filter(BothDiab, DiabeticDateDiff<365.25 & DiabeticDateDiff>-365.25))/nrow(BothDiab)
Perc_10year<- nrow(filter(BothDiab, DiabeticDateDiff<3652.5/2 & DiabeticDateDiff>-3652.5/2))/nrow(BothDiab)
ggplot(Perc_ShiftAge) + geom_point(aes(x= Age5, y = `sum(ComorbShift)/dplyr::n()`))
...
Volume_Cancer<- inner_join(Diabetic_All_Tab , Volume_Patients, by = c("Study_ID"))

# Create Tables for patient numbers by method of identification 

AllBloodOnly2<- nrow(dplyr::filter(Volume_Cancer, (Diagnostic == 1 | Late.Diagnostic == 1) &
                                    T1Diabetes == 0 & T2Diabetes ==0 & OtherDiabetes ==0 &
                                    Late.T1Diabetes ==0 & Late.T2Diabetes ==0 & Late.OtherDiabetes ==0))

AllCodedOnly2<- nrow(dplyr::filter(Volume_Cancer,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 |
                                                      Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                    Diagnostic == 0 & Late.Diagnostic == 0 ))

AllBoth2 <- nrow(dplyr::filter(Volume_Cancer,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 |
                                                  Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                (Diagnostic == 1 | Late.Diagnostic == 1 )))


PreBloodOnly2<- nrow(dplyr::filter(Volume_Cancer, (Diagnostic == 1 ) &
                                    T1Diabetes == 0 & T2Diabetes ==0 & OtherDiabetes ==0))

PreCodedOnly2<- nrow(dplyr::filter(Volume_Cancer,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 ) &
                                    Diagnostic == 0 ))

PreBoth2 <- nrow(dplyr::filter(Volume_Cancer,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 ) &
                                (Diagnostic == 1 )))


PostBloodOnly2<- nrow(dplyr::filter(Volume_Cancer, (Late.Diagnostic == 1) &
                                     Late.T1Diabetes ==0 & Late.T2Diabetes ==0 & Late.OtherDiabetes ==0))

PostCodedOnly2<- nrow(dplyr::filter(Volume_Cancer,( Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                     Late.Diagnostic == 0 ))

PostBoth2 <- nrow(dplyr::filter(Volume_Cancer, (Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                                 Late.Diagnostic == 1 ))
...
Filtered_Coding_Only<- dplyr::filter(Volume_Cancer,(T1Diabetes == 1 | T2Diabetes ==1 | OtherDiabetes ==1 |
                               Late.T1Diabetes ==1 | Late.T2Diabetes ==1 | Late.OtherDiabetes ==1) &
                (Diagnostic == 0 | Late.Diagnostic == 0 )) %>%
...
Publication_Bar_Tab<- as.data.frame(rbind(cbind(as.numeric(c(AllBloodOnly, AllCodedOnly, AllBoth, AllBloodOnly2, AllCodedOnly2, AllBoth2)),
      rep(c("Uniquely Identified by HBA1c", "Uniquely Identified by Clinical Coding", "Universally Identified"), times = 2),
rep(c("All", "LTHT Laboratory Catchment"), each = 3),
rep("Hybrid", times = 6)),
cbind(as.numeric(c(AllCodedOnly, AllBoth,  AllCodedOnly2, AllBoth2)),
      rep(c( "Uniquely Identified by Clinical Coding", "Universally Identified"), times = 2),
      rep(c("All", "LTHT Laboratory Catchment"), each = 2),
      rep("Clinical Coding Only", times = 4)),
cbind(as.numeric(c(AllBloodOnly, AllBoth, AllBloodOnly2, AllBoth2)),
      rep(c("Uniquely Identified by HBA1c", "Universally Identified"), times = 2),
      rep(c("All", "LTHT Laboratory Catchment"), each = 2),
      rep("Abnormal HBA1c Only", times = 4)))) %>%
  `colnames<-`(c("Patients", "Route", "Cohort", "Definition" )) %>%
  mutate(Total = ifelse(Cohort == "All", TotalAll, TotalAll2)) %>%
  mutate(Percentage = round(as.numeric(as.character(Patients))/as.numeric(as.character(Total))*100, digits =1)) %>%
  dplyr::arrange(desc(Route), .by_group = T) %>% 
  dplyr::group_by(Cohort, Definition) %>%
  dplyr::mutate(Position =  cumsum(Percentage)- (0.5 * Percentage)) %>%# error with ordering in all cohort makes labels in wrong order so manually overwritten
mutate(Label = paste(as.character(Patients), "\n", "(", as.character(Percentage), "%)", sep =""))
 
  

Publication_Bar<- 
  ggplot(Publication_Bar_Tab) +
  geom_bar(aes(x= Definition, y = as.numeric(as.character(Percentage)), fill = Route), stat= "identity") +
  geom_text(aes(x= Definition, y = Position, label = Label), color = "white", size = 8)+
  ylab("Percentage of Total Identified Diabetic Population") + 
  xlab("Diabetes Definition") +
...
Publication_Bar_BW<- ggplot(Publication_Bar_Tab) +
  geom_bar(aes(x= Definition, y = as.numeric(as.character(Percentage)), fill = Route), stat= "identity") +
  geom_text(aes(x= Definition,y = Position,label = Label), color = "white", size = 8)+
  ylab("Percentage of Total Identified Diabetic Population") + 
  xlab("Diabetes Definition") +
  scale_fill_discrete(name = "Subgroup")+
  facet_wrap(.~Cohort) +
  scale_fill_grey(start = 0, end = .7, name = "Subgroup")+
  theme(text = element_text(size = 30),
        panel.background = element_rect(fill = "white",
                                        size = 2, linetype = "solid"),
        panel.grid.major = element_line(size = 0.5, linetype = 'solid',
